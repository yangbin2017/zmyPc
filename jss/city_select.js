...
// 默认值
    var city_json = {
        "citylist": [{
            ...
            "c": [{
                "n": "成都",
                "a": [{"s": "锦江区"}, {"s": "青羊区"}, {"s": "金牛区"}, {"s": "武侯区"}, {"s": "成华区"}, {"s": "高新区"}, {"s": "龙泉驿区"}, {"s": "青白江区"}, {"s": "新都区"}, {"s": "温江区"}, {"s": "金堂县"}, {"s": "双流县"}, {"s": "郫县"}, {"s": "大邑县"}, {"s": "蒲江县"}, {"s": "新津县"}, {"s": "都江堰市"}, {"s": "彭州市"}, {"s": "邛崃市"}, {"s": "崇州市"}]
            }, {
                "n": "自贡",
    ...
    var special_prov = ['香港', '澳门', '北京', '上海', '重庆', '天津', '台湾'];
// 遍历赋值省份下拉列表
    var province_html = '<ul class="area-list">';
    $.each(city_json.citylist, function (i, prov) {
        var prov_name = prov.p;
        if (special_prov.indexOf(prov_name) < 0) {
            prov_name = prov_name + '省';
        }
        province_html += '<li><a href="javascript:;" data-value="' + i + '">' + prov_name + '</a></li>';
    });
    province_html += '</ul>';
    ...
    var provinceHtml = '<div class="content"><div data-widget="tabs" class="m JD-stock" id="JD-stock">'
        + '<div class="mt">'
        + '    <ul class="tab">'
        + '        <li data-index="0" data-widget="tab-item" class="curr"><a href="javascript:;" class="hover" data-id=""><em>请选择</em><i></i></a></li>'
        + '        <li data-index="1" data-widget="tab-item" style="display:none;"><a href="javascript:;" class="" data-id=""><em>请选择</em><i></i></a></li>'
    ...
    var $selector_obj = $("#store-selector");
    $selector_obj.find(".text").after(provinceHtml);
    var areaTabContainer = $selector_obj.find("#JD-stock .tab li");
    var provinceContainer = $selector_obj.find("#stock_province_item");
    var cityContainer = $selector_obj.find("#stock_city_item");
    var areaContainer = $selector_obj.find("#stock_area_item");

    (function () {
        $selector_obj.click(function () {
            $selector_obj.addClass('hover');
            $selector_obj.find(".content,#JD-stock").show();
            chooseProv();
            $selector_obj.find('.close-selector').click(function () {
                $selector_obj.removeClass('hover');
                $selector_obj.find(".content").hide();
                return false;
            });
        }).find("dl").remove();
    ...
    function chooseProv() {
        ...
        provinceContainer.find("a").click(function () {
            ...
            getCityList($(this).attr("data-value"));
            areaTabContainer.removeClass("curr");
            areaTabContainer.eq(1).addClass("curr").show();
            provinceContainer.hide();
            cityContainer.show();
            areaContainer.hide();
            $('.prov').val($(this).text());
            areaTabContainer.eq(0).find('em').text($(this).text());
            areaTabContainer.eq(0).find('a').attr('data-id', $(this).attr("data-value"));
            areaTabContainer.eq(1).find('em').text('请选择');
            chooseCity();
        });
    }
    ...
    function chooseCity() {
        ...
        cityContainer.find("a").click(function () {
            getCityArea(areaTabContainer.eq(0).find('a').attr('data-id'), $(this).attr("data-value"));
            areaTabContainer.removeClass("curr");
            areaTabContainer.eq(2).addClass("curr").show();
            provinceContainer.hide();
            cityContainer.hide();
            areaContainer.show();
            $('.city').val($(this).text());
            areaTabContainer.eq(1).find('em').text($(this).text());
            areaTabContainer.eq(1).find('a').attr('data-id', $(this).attr("data-value"));
            areaTabContainer.eq(2).find('em').text('请选择');
            chooseArea();
        });
    }
    ...
    function chooseArea() {
        ...
        areaContainer.find("a").click(function () {
            areaTabContainer.eq(2).find('em').text($(this).text());
            var select_prov = areaTabContainer.eq(0).find('em').text();
            var select_city = areaTabContainer.eq(1).find('em').text();
            var select_area = areaTabContainer.eq(2).find('em').text();
            $('.area').val($(this).text());
            $selector_obj.find(".text div").text(select_prov + ' - ' + select_city + ' - ' + select_area + ' ');
            $selector_obj.removeClass('hover');
            $selector_obj.find(".content").hide();
            $(".city-checktip").html('').removeClass('Validform_error').addClass('Validform_right');
            return false;
        });
    }

    function getCityList(provinceId) {
        var city_arr = city_json.citylist[provinceId].c;
        var city_html = '<ul class="area-list">';
        $.each(city_arr, function (i, city) {
            var city_name = city.n;
            if (special_prov.indexOf(city_json.citylist[provinceId].p) < 0) {
                city_name = city_name + '市';
            }
            city_html += '<li><a href="javascript:;" data-value="' + i + '">' + city_name + '</a></li>';
        });
        city_html += '</ul>';
        cityContainer.html(city_html);
    }

    function getCityArea(province_id, city_id) {
        var area_arr = city_json.citylist[province_id].c[city_id].a;

        var area_html = '<ul class="area-list">';
        $.each(area_arr, function (i, area) {
            var area_name = area.s;
            area_html += '<li><a href="javascript:;" data-value="' + i + '">' + area_name + '</a></li>';
        });
        area_html += '</ul>';
        areaContainer.html(area_html);
    }
